...
$(document).ready(function(){
	...
	$("#searchForm").on("submit",function(){
		...
		$(this).append("<input type='hidden' name='theme' value='${requestScope.theme}'>");
				...
				<c:forEach items="${requestScope.restaurantList }" var="restaurant">
					<tr class="selectList" align="center">
						...
						<td align="left" style="border-right-style:hidden;">
							<a href="${initParam.rootPath }/restaurant/restaurantView.do?restaurantNo=${restaurant.restaurantNo }&backURL=${initParam.rootPath}/restaurant/showListByTheme.do?theme=${requestScope.theme}%26align=${requestScope.align}%26currentPage=${requestScope.pagingBean.currentPage}%26searchWord=${requestScope.searchWord}">${restaurant.restaurantName}</a>
...
<c:choose>
	<c:when test="${pagingBean.previousPageGroup }">
		<a href="${initParam.rootPath }/restaurant/showListByTheme.do?currentPage=${pagingBean.startPageOfPageGroup-1}&theme=${requestScope.theme}&align=${requestScope.align}&searchWord=${requestScope.searchWord}">&lt;</a>
	</c:when>
	<c:otherwise>
		&lt;
	</c:otherwise>
...
<c:forEach begin="${pagingBean.startPageOfPageGroup }" end="${pagingBean.endPageOfPageGroup}" var="pageNum">
	<c:choose>
		...
		<td class="pagingNo" style="padding:1px;">
			&nbsp;
			<a href="${initParam.rootPath }/restaurant/showListByTheme.do?currentPage=${pageNum}&theme=${requestScope.theme}&align=${requestScope.align}&searchWord=${requestScope.searchWord}">${pageNum}</a>
			&nbsp;
		</td>
		</c:otherwise>
	</c:choose>
...
<c:choose>
	<c:when test="${pagingBean.nextPageGroup }">
		<a href="${initParam.rootPath }/restaurant/showListByTheme.do?currentPage=${pagingBean.endPageOfPageGroup+1}&theme=${requestScope.theme}&align=${requestScope.align}&searchWord=${requestScope.searchWord}">&gt;</a>
	</c:when>
	<c:otherwise>
		&gt;
	</c:otherwise>
